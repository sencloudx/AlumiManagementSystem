 ...
 * */
public class ClassesExcelBatchInputImpl implements ExcelBatchInput {
	private Sheet sheet;
	...
	private boolean validateNumRepeat(int i,String[] DBValue){
		...
		List<Classes> classesList = classesDAO.findListByHQL(hql);
		for(Classes classes: classesList){
			if(classes.getClassesName().equals(DBValue[0]) && DBValue[0]!=""){
				bolValidate=false;
				msg.append("错误信息：第"+i+"班级的名称与数据库已存储的重复,该名称是["+DBValue[0]+"],请检查！</br>");
				return bolValidate;
			}
		}
		return bolValidate;
	...
	private void finalInsert(String[] DBValue){
		...
		classes.setClassesName(DBValue[0]);
		classes.setClassesRemarks(DBValue[1]);
